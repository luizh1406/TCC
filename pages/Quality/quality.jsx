...
import {loadingBox, loadingLabel} from "../../src/styles/containers/containers";
...
async function saveCk(ckGroup, ckFamily, ckName, ckSetor, questionCont, setLoading, setQuestionCont){
  setLoading(true);
  ...
  if(ckGroup === "" || ckFamily === "" || ckName === "" || ckSetor === ""){
    ...
    while (cont < questionCont){
      if (document.getElementById(`d-${cont}`)){
        ...
        questions.push({nome: ckName, linha: ckGroup, familia: ckFamily, setor: ckSetor, sequencia:sequencia, descricao: descricao, preenchimento: preenchimento, codigo_pergunta: `d-${cont}`});
      }
      cont++;
    }
  }
    ...
    <>
      <div style={{flexDirection:'column', display:'flex', height: '100vh'}}>
          <div style={{...st_topDiv}}>
            ...
            <label style={{display: 'flex', fontSize:25, fontFamily: "'Montserrat', sans-serif", color: "white", width: '100%', justifyContent: "center", alignItems: "center"}}>NEXOS - JIMP</label>
            <button
              style={st_logoutBtn}
        ...
        <div style={{...mainBkgImage}}>
          <div style={{...st_translucedDiv}}>
            <label style={{...st_moduleTitle}}>Parâmetros do módulo de qualidade</label>
            <div style={{...st_translucedBox, marginTop: '10px', flexDirection: 'column'}}>
              <label style={{width: '100%', height: '30px', padding: '10px', borderBottom: '2px solid white', boxSizing: 'border-box', display: 'flex', alignItems: 'center'}}
              >Cadastro de checklist</label>
              <div style={{display: 'flex', flexDirection: 'row', width: '100%', height: '100%', marginTop: '15px'}}>
                <div style={{display: 'flex', flexDirection: 'column', width: '100%', height: '100%'}}>
                  <div style={{...st_rowDiv}}>
                    <label style={{width: '100%'}}>Informe para qual linha de produto:</label>
                    <select
                    id='ck-group'
                    style={{width: '100%'}}
                    onChange={()=> setCkGroup(document.getElementById('ck-group').value)}>
                      <option>Semirreboque</option>
                      <option>Sobrechassi</option>
                    </select>
                  </div>
                  <div style={{...st_rowDiv}}>
                    <label style={{width: '100%'}}>Informe a familia de produto:</label>
                    <select
                    id="ck-family"
                    style={{width: '100%'}}
                    onChange={()=> setckFamily(document.getElementById('ck-family').value)}>
                ...
                <div style={{display: 'flex', flexDirection: 'column', width: '100%', height: '100%', marginRight: '15px'}}>
                  <div style={{...st_rowDiv}}>
                    <label style={{width: '100%', paddingLeft: '10px', textAlign:'right', paddingRight: '15px'}}>Nome do checklist:</label>
                    <input
                    id="ck-name"
                    style={{width: '100%'}}
                    onChange={()=> setCkName(document.getElementById("ck-name").value)}/>
                  </div>
                  <div style={{...st_rowDiv}}>
                    <label style={{width: '100%', paddingLeft: '10px', textAlign:'right', paddingRight: '15px'}}>Setor:</label>
                    <input
                    id="ck-setor"
                    style={{width: '100%'}}
                    onChange={()=> setCkSetor(document.getElementById("ck-setor").value)}/>
                  </div>
                </div>
              </div>
              <label style={{width: '100%', height: '40px', padding: '10px', borderTop: '2px solid white', borderBottom: '2px solid white', boxSizing: 'border-box', display: 'flex', alignItems: 'center'}}
              >Cadastro de checklist</label>
              <div id="questions-table" style={{display: 'flex', flexDirection: 'column', width: '100%', height: '100%'}}>
                <div style={{...st_rowDiv}}>
                  <button style={{...st_button}} onClick={()=> addQuestion()}>Adicionar perguntas</button>
                </div>
                <div style={{...st_rowDiv}}>
                  <label style={{width: '100%', height: '30px', justifyContent: 'center', backgroundColor: "#010B40", display: 'flex', alignItems: 'center'}}>Sequência das perguntas</label>
                  <label style={{width: '100%', height: '30px', justifyContent: 'center', backgroundColor: "#010B40", display: 'flex', alignItems: 'center'}}>Descrição da revisão</label>
                  <label style={{width: '100%', height: '30px', justifyContent: 'center', backgroundColor: "#010B40", display: 'flex', alignItems: 'center'}}>Preenchimento</label>
                  <label style={{width: '10%', height: '30px', justifyContent: 'center', backgroundColor: "#010B40", display: 'flex', alignItems: 'center',marginRight: '15px'}}></label>
                </div>
              </div>
              {questionCont > 1 && (
                  <>
                    <div style={{...st_rowDiv}}>
                      <button style={{...st_button}} onClick={()=> saveCk(ckGroup, ckFamily, ckName, ckSetor, questionCont, setLoading, setQuestionCont)}>Gravar</button>
                    </div>
                  </>
                )}
            </div>
